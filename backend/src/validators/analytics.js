...
const summaryQuerySchema = z
  .object({
    storeId: z
      .string({ required_error: 'storeId is required' })
      .regex(OBJECT_ID_RE, 'Invalid storeId'),
    startDate: z.string().datetime({ offset: true, message: 'startDate must be an ISO 8601 datetime' }).optional(),
    endDate:   z.string().datetime({ offset: true, message: 'endDate must be an ISO 8601 datetime' }).optional(),
